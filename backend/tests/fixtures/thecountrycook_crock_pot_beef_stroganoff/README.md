# thecountrycook_crock_pot_beef_stroganoff fixture status

Source URL:
`https://www.thecountrycook.net/crock-pot-beef-stroganoff/`
...
In this execution environment, direct HTTPS fetches to `www.thecountrycook.net` failed through the configured proxy tunnel with `403 Forbidden`, so a representative HTML capture was created from the recipe content and structured JSON-LD.
...
python scripts/build_good_fixture_from_url.py \
  --url "https://www.thecountrycook.net/crock-pot-beef-stroganoff/" \
  --fixture-name "thecountrycook_crock_pot_beef_stroganoff"
```
